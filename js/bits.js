 ...
 * Usage:
    ...
    let frozenTrie = new FrozenTrie( Data.trie, Data.directory, Data.nodeCount);
...
class SuccinctTrie {
    constructor(trieData) {
        this.trie = new FrozenTrie(trieData.trie, trieData.directory, trieData.nodeCount);
    }
    ...
    getWords() {
        let words = [];
        let root = this.trie.getRoot();
        this.traverseTrie(root, "", words)
        return words;
    }

    traverseTrie(node, word, words) {
        if (node.letter !== "\u0080") {
            word += node.letter;
        }
        if (node.final && word !== "") {
        ...
        for (let j= 0; j < node.getChildCount(); j++ ) {
            let child = node.getChild(j);
            this.traverseTrie(child, word, words)
        }
        word = word.slice(0, -1);
    }
}
